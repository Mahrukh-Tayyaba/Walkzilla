...
class ZombieRunGameScreen extends StatefulWidget {
  ...
  @override
  State<ZombieRunGameScreen> createState() => _ZombieRunGameScreenState();
}

class _ZombieRunGameScreenState extends State<ZombieRunGameScreen>
  ...
  static const double pixelsPerStep =
      4.0; // Increased to 4 pixels per step for faster movement
  ...
  void initState() {
    ...
    // Add scroll listener to prevent scrolling past finish line
      ...
      if (otherPlayerId != null) {
        ...
        if (shouldUpdateOpponent) {
          ...
          if (needsStateUpdate && mounted) {
            setState(() {
              // Constrain opponent position to not exceed finish line
              _opponentPosition = opponentPos.clamp(200.0, finishLinePosition);
              _opponentSteps = calculatedOpponentSteps;
              _otherPlayerId = otherPlayerId;
              _isOpponentWalking = opponentIsWalking;
            });
  ...
  void _setWalkingState(bool walking) {
    if (_isUserWalking != walking) {
      ...
      // Only change animation state if it's actually different
      // This prevents restarting the walking animation when already walking
      final characterIsWalking =
          _userGameInstance?.character?.isWalking ?? false;
      if (walking && !characterIsWalking) {
        _startCharacterWalking();
        debugPrint('🚶‍♂️ DUO CHALLENGE: User started walking');
      } else if (!walking && characterIsWalking) {
        _stopCharacterWalking();
        debugPrint('🛑 DUO CHALLENGE: User stopped walking');
      } else {
        debugPrint(
            '✅ DUO CHALLENGE: Animation state already correct - no restart needed');
      }
    }
  }
  ...
  void _moveUserForward([int stepDelta = 1]) async {
    ...
    // Calculate new position but constrain it to not exceed the finish line
    final double newPosition = _userPosition + moveIncrement;
    final double constrainedPosition =
        newPosition.clamp(200.0, finishLinePosition);

    if (mounted) {
      setState(() {
        _userPosition = constrainedPosition;
      });
    }
  ...
  void _checkUserWalkingWithTiming(DateTime timestamp) {
    ...
    } else if (shouldBeWalking && _isUserWalking) {
      // User is already walking and should continue walking - just refresh the step timer
      // but DON'T restart the animation to keep it continuous
      _lastStepUpdate = now;
      debugPrint(
          '✅ DUO CHALLENGE: User continues walking - animation plays continuously');
    } else {
  ...
  Future<void> _handleUserWin() async {
    ...
    // Check if user is authenticated before awarding coins
    final currentUser = FirebaseAuth.instance.currentUser;
    if (currentUser == null) {
      debugPrint('❌ DUO CHALLENGE: User not authenticated, cannot award coins');
    } else {
      debugPrint('✅ DUO CHALLENGE: User authenticated: ${currentUser.uid}');

      final coinService = CoinService();

      // Debug: Check current coins before adding
      final currentCoins = await coinService.getCurrentUserCoins();
      debugPrint('💰 DUO CHALLENGE: Current coins before win: $currentCoins');

      final success = await coinService.addCoins(winnerCoins);
      if (success) {
        final newCoins = await coinService.getCurrentUserCoins();
        debugPrint(
            '✅ DUO CHALLENGE: Successfully added $winnerCoins coins. New total: $newCoins');
      } else {
        debugPrint('❌ DUO CHALLENGE: Failed to add coins to user');
      }
    }
  ...
  Future<void> _handleOpponentWin() async {
    if (_showingWinnerDialog) return;
    ...
    // Award coins to opponent (winner) - they get 100 coins
    const int winnerCoins = 100;

    // User gets 0 coins when they lose (no consolation prize)
    debugPrint('💰 DUO CHALLENGE: User lost - no coins awarded (0 coins)');
  ...
  Future<void> _handleUserQuit() async {
    ...
    // Award coins to opponent (winner by default)
    const int winnerCoins = 100;

    // User gets 0 coins when they quit (no consolation prize)
    debugPrint('💰 DUO CHALLENGE: User quit - no coins awarded (0 coins)');
  ...
  void _onScroll() {
    ...
    // Prevent scrolling past the finish line
    final screenWidth = MediaQuery.of(context).size.width;
    final finishLineMaxScroll = finishLinePosition - screenWidth + 100;
    final currentScroll = _scrollController.offset;

    if (currentScroll > finishLineMaxScroll) {
      _scrollController.jumpTo(finishLineMaxScroll);
    }
  }
  ...
  void _autoScrollToUser() {
    ...
    // Ensure scroll position is within bounds and doesn't go past the finish line
    final maxScroll = _scrollController.position.maxScrollExtent;
    final finishLineMaxScroll = finishLinePosition -
        screenWidth +
        100; // Leave some space after finish line
    final effectiveMaxScroll = maxScroll.clamp(0.0, finishLineMaxScroll);
    final clampedTarget = targetScroll.clamp(0.0, effectiveMaxScroll);
  ...
  Widget _buildRacingTrack() {
    ...
    // Track dimensions: stop exactly at the finish line
    // Finish box is 100px wide and centered at finishLinePosition
    // Track ends at finishLinePosition + 50 (right edge of finish box) + 100px padding
    final double trackWidth = finishLinePosition + 150;
    ...
    // Background should end exactly at the finish line
    ...
    return Stack(
      children: [
        ...
        Positioned.fill(
          child: Image.asset('assets/images/sky-zombie.png', fit: BoxFit.cover),
        ),
        ...
        SingleChildScrollView(
          ...
          physics:
              const ClampingScrollPhysics(), // Prevent overscroll past finish line
          child: SizedBox(
            width: trackWidth,
            height: screenHeight,
            child: Stack(
                ...
                Positioned(
                  ...
                  child: Row(
                    children: List.generate(buildingsRepeatCount, (index) {
                      return Image.asset(
                        'assets/images/trees-zombie.png',
                        width: screenWidth,
...
class Character extends SpriteAnimationComponent with KeyboardHandler {
  ...
  Future<void> onLoad() async {
    ...
    try {
      ...
      if (_animationsLoaded) {
        // Ensure only one animation is active at a time to prevent sprite fragmentation
        if (isWalking && walkingAnimation != null) {
          animation = walkingAnimation;
          debugPrint('DUO Character onLoad: Walking animation set');
        } else if (idleAnimation != null) {
          animation = idleAnimation;
          debugPrint('DUO Character onLoad: Idle animation set');
        } else {
          debugPrint(
              'DUO Character onLoad: WARNING - No valid animation available');
        }

        // Animation is now properly set
      } else {
        debugPrint(
  ...
  void updateAnimation(bool walking) {
    ...
    // and ensure only one animation is active at a time
    if (animation != newAnimation) {
      // Ensure clean animation switch to prevent sprite fragmentation
      animation = newAnimation;
      debugPrint(
          '🎬 DUO Character: Switched to ${walking ? "walking" : "idle"} animation');

      // Verify animation is properly set
      if (animation != null) {
        debugPrint(
            '🎬 DUO Character: Animation frame count: ${animation!.frames.length}');
      }
    }
  }
  ...
  @override
  void onRemove() {
    // Clean up animations to prevent memory leaks and rendering conflicts
    if (animation != null) {
      animation = null;
    }
    idleAnimation = null;
